...
)
{
	...
	while ((*p != NULLCH) && (*p != SH_NEWLINE))
	{
		...
		while (((ch = *p) != SH_NEWLINE) && (ch != NULLCH) && (ch != SH_LESS) && (ch != SH_GREATER) && (ch != SH_BLANK) && (ch != SH_TAB) && (ch != SH_AMPER) && (ch != SH_SQUOTE) && (ch != SH_DQUOTE))
		{
			tokbuf[tbindex++] = ch;
			p++;
		}
		...
		if ((ch == SH_SQUOTE) || (ch == SH_DQUOTE) || (ch == SH_LESS) || (ch == SH_GREATER))
		{
			return SYSERR;
		}
